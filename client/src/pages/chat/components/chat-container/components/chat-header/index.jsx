...
const ChatHeader = () => {
  ...
  return (
    <div 
    className='h-[10vh] border-b-2 border-[#2f203b] flex items-center justify-between px-20'
    >
                ...
                <div className='w-12 h-12 relative'>
                  {
                    selectedChatType === "contact" ? (<Avatar className="h-12 w-12 rounded-full overflow-hidden" >
                    {
                      selectedChatData.image ? <AvatarImage src={`${HOST}/${selectedChatData.image}`} alt="profile" className="object-cover w-full h-full bg-black" /> :
                        <div
                          className={`uppercase w-12 h-12 text-lg border-[1px] flex items-center justify-center rounded-full ${getColors(selectedChatData.color)}`}

...
const MessageContainer = () => {
  ...
  useEffect(() => {
    ...
    // console.log("seletedChatData",{selectedChatData})
  ...
  const checkIfImage = (filePath) => {
    const imageRegex = /\.(jpeg|jpg|png|gif|bmp|tiff|webp|svg|ico|heic|heif)$/i
      ...
      return (
        <div key={index}>
          ...
          </div>)}
          {
            ...
            selectedChatType==="channel" && renderChannelMessaegs(message)
          }
        </div>
      )
    })
  ...
  const renderDmMessages = (message) => (
    <div className={`${message.sender===selectedChatData._id ? "text-left" : "text-right"}`}>
      ...
      "bg-[#8517ff]/5 text-[#8417ff]/90 border-[#8417ff]/50 " 
        ...
        }
        {
          ...
          }>
          {
            checkIfImage(message.fileUrl) ? (<div onClick={()=>{
              ...
              }} className='cursor-pointer'><img src={`${HOST}/${message.fileUrl}`} height={300} width={300} /></div>) : 
            (<div className='flex items-center justify-center gap-4'>
              <span className='text-white/8- text-3xl bg-black rounded-full p-3'>
                <MdFolderZip/>
              </span>
  ...
  const renderChannelMessaegs = (message) => {
    return (
        ...
        <Avatar className="h-8 w-8 rounded-full overflow-hidden" >
                    {
                      message.sender.image && (<AvatarImage src={`${HOST}/${message.sender.image}`} alt="profile" className="object-cover w-full h-full bg-black" />)
                        ...
                        <AvatarFallback
                          className={`uppercase w-8 h-8 text-lg flex items-center justify-center rounded-full ${getColors(message.sender.color)}`}
                        >{message.sender.firstName ? message.sender.firstName.split("").shift() : (message.sender.email).split("").shift()}</AvatarFallback>
    ...
    <div className='flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:w-[80vw] w-full'>
      ...
      <div ref={scrollRef} />
      {
        showImage && <div className='fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-lg flex-col'>
          ...
          <div className='flex gap-5 fixed top-0 mt-0'>
            <button 
